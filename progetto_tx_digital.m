...
%% STREAMING
samples_per_block = round(audio_fs_target * buffer_audio_sec);  %converto 
                                                                % tempo in
                                                                %campioni
                                                                %audio
num_blocks = ceil(length(audio)/samples_per_block); %calcolo il numero di 
                                                    %blocchi che servono 
                                                    %per coprire tutta 
                                                    %la canzone
...
for k = 1:num_blocks
    ...
    bits = bits.';
    bits = bits(:);                            %ora ho un flusso binario
    bits = bits(1:2*floor(length(bits)/2));
                                               %butto ultimo bit se numero
                                               %dispari perchè ne ho
                                               %bisogno due per simbolo in
                                               %qpsk
    % QPSK
    sym = reshape(bits, 2, []).';
    ...
    qpsk = qpsk / sqrt(2);
                                               ...
                                               % complessi IQ
    % Pulse shaping
    txSig = upfirdn(qpsk, rrc, sps);           %upsampling (inserisco zeri),
                                               %filtro con rrc e limito banda
    txSig = 0.7 * txSig / max(abs(txSig));     %normalizzo

    % Se l'ultimo blocco è più corto, lo riempiamo di zeri
    expected_len = samples_per_block * sps;
    if length(txSig) < expected_len
        txSig(end+1:expected_len) = 0;
    end
    % TX
    step(tx, txSig);
end
